...
const { mergeUserPreProcessorConfig } = require('./userConfig/userPreProcessorConfig')
...
const isAnalysisEnabled = process.env.USE_ANALYZE

const getTerserPluginOptions = (isEnvProductionProfile) => {
  const esbuildMinifyOption = {
    minify: TerserPlugin.esbuildMinify,
    terserOptions: {}
  }

  const swcMinifyOption = {
  ...
  return process.env.USE_ESBUILD
    ? esbuildMinifyOption
    : process.env.USE_SWC
    ? swcMinifyOption
    : terserOptions
  ...
  return {
    ...
    optimization: {
      ...
      minimizer: [
        ...
        new TerserPlugin(getTerserPluginOptions(isEnvProductionProfile)),
        // This is only used in production mode
        // andy end
    ...
    plugins: [
      ...
      // andy start
      isAnalysisEnabled && new BundleAnalyzerPlugin(),
...
module.exports = {
  ...
  dev: async (webpackEnv, mfsu) => {
    const config = mfsu
      ? getOriginWebpackConfig(webpackEnv, mfsu)
      : getOriginWebpackConfig(webpackEnv)
